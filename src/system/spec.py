...
class DummySystem():
    ...
    def __init__(
        ...
        self.base_lr = optimizer_config.get('lr', 0.00001) if optimizer_config else 0.00001
        self.warmup_epochs = trainer_config.get('warmup_epochs', 10)
    ...
    def _step_lr(self, epoch):
        if epoch < self.warmup_epochs:
            ...
            progress = (epoch - self.warmup_epochs) / max(1, self.epochs - self.warmup_epochs - 1)
            lr = 1e-7 + (self.base_lr - 1e-7) * (1 + math.cos(math.pi * progress)) / 2
