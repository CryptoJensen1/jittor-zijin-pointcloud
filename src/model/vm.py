...
class VelocityModule(ModelSpec):
    ...
    def __init__(self, model_config, transform_config):
        ...
        self.distance_module = None  # DistanceModule, set externally for inference

    def set_distance_module(self, dm):
        self.distance_module = dm
        dm.eval()
    ...
    def denoise_langevin_dynamics(self, pcl_noisy, num_steps: int=10):
        """
        pcl_noisy: (B, N, 3)
        """
        ...
        with jt.no_grad():
            ...
            for it in range(num_steps):
                ...
                if self.distance_module is not None:
                    d_phi = self.distance_module.forward(pcl_next)  # (B, 1)
                    step = d_phi.unsqueeze(-1) / num_steps  # → (B, 1, 1)
                else:
                    step = 1.0 / num_steps
...
def patch_based_denoise(model: VelocityModule, pcl_noisy, patch_size=1000, seed_k=6, seed_k_alpha=1) -> jt.Var:
    ...
    pcl_out = []
    pcl_noisy_sqz = pcl_noisy.squeeze(0)  # (N, 3)
    for pidx in range(N):
        ...
        if mask.sum() > 0:
            ...
            pcl_out.append(pcl_noisy_sqz[pidx].unsqueeze(0))
