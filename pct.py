...
# Force Jittor to use fallback softmax (avoids CUB kernel incompatibility on RTX 50)
import jittor.other.code_softmax as code_softmax
code_softmax.can_softmax_v1 = lambda a, dim: False
...
class ModelNet40Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            # 1. Y-axis full rotation + X/Z small perturbation
            angle_y = np.random.uniform(0, 2 * np.pi)
            angle_x = np.random.uniform(-np.pi/18, np.pi/18)  # ±10°
            angle_z = np.random.uniform(-np.pi/18, np.pi/18)  # ±10°
            ...
            points = points @ Ry.T @ Rx.T @ Rz.T
            # 2. Random isotropic scaling (same factor for all axes)
            scale = np.float32(np.random.uniform(0.85, 1.15))
            points = points * scale
            # 3. Random point jitter (Gaussian noise, sigma=0.01)
            jitter = np.random.randn(*points.shape).astype(np.float32) * 0.01
...
class CosineAnnealingLR:
    def __init__(self, optimizer, T_max, eta_min=1e-5, warmup_epochs=10):
...
def train_one_epoch(model, train_loader, optimizer, epoch, log_interval=20):
    ...
    for batch_idx, (points, labels) in enumerate(train_loader):
        points = np.array(points).transpose(0, 2, 1)  # numpy transpose (B, 3, N)
        points = jt.array(points)
        labels = jt.array(labels).reshape(-1)

        logits = model(points)
        n_classes = logits.shape[1]
        smooth_labels = jt.nn.one_hot(labels, n_classes) * 0.9 + 0.1 / n_classes
