"""Predict with V7 big model checkpoint_59."""
import os, sys, time
import numpy as np
import jittor as jt
...
def main():
    vm_ckpt = "experiments/vm_big/checkpoint_59.pkl"
    test_dir = "dataset_test_noisy/shapenet"
    output_dir = "tmp_predict/v7"

    vm_cfg = OmegaConf.to_container(OmegaConf.load("configs/model/vm_big.yaml"))
    transform_cfg = OmegaConf.to_container(OmegaConf.load("configs/transform/vm.yaml"))

    print(f"Loading V7 model: {vm_cfg['feat_embedding_dim']}dim, {vm_cfg['decoder_hidden_dim']}hidden")
    print(f"Checkpoint: {vm_ckpt}")
